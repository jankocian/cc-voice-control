...
// same-origin bridge WebSocket (connect-src 'self'). TTS plays through a native <audio> element fed by
// (Managed)MediaSource — `blob:` in media-src is the MediaSource object URL, `data:` is the silent-WAV
// that unlocks iOS autoplay on first tap. No WASM and no blob worklet/worker (mediabunny re-containerizes
// AAC to fragmented MP4 in pure JS), so script-src is plain 'self'.
const SESSION_CSP = [
  "default-src 'self'",
  "script-src 'self'",
  "style-src 'self'",
