        ...
        req.on("end", () => {
          ...
          try {
            if (route === "/turn-open") {
              ...
              // A pending question already shows "awaiting" from the transcript/overlay, and clears itself when
              // the answer lands. Don't ALSO arm the sticky permission flag for it — that flag clears only on a
              // later edge (Stop/next tool), so it would leave the lamp stuck on "needs you" after you answered.
              // Arm it only for a genuine tool-permission prompt (no open question).
