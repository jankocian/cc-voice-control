...
own daemon, which writes its `pid` into a **per-pane** runtime file at a fixed, plugin-data-independent
path (`$HOME/.cache/cc-voice-control/runtime/<CMUX_SURFACE_ID>.json`). Sending that pid `SIGTERM` runs
its clean-shutdown handler (drops its thread at the bridge, closes the WebSocket, removes its runtime
file). Sibling panes are untouched — this stops only the voice remote for the current pane.
